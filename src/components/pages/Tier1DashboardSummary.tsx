  ...
  const systems = [
    {
      ...
      icon: Target,
      score: d.belief?.score?.overallScore || 50,
      level: d.belief?.score?.level || 1,
    ...
    },
    {
      ...
      icon: Brain,
      score: d.mindset?.pillars?.reduce((acc: number, p: {currentScore: number}) => acc + p.currentScore, 0) / (d.mindset?.pillars?.length || 1) || 50,
      count: d.mindset?.pillars?.length || 0,
      color: 'text-blue-600',
      bgColor: 'bg-blue-500',
    },
    {
      ...
      icon: Zap,
      score: d.confidence?.cq?.overallCq || 60,
      domains: 6,
    ...
    },
    {
      name: 'Journaling',
      icon: BookOpen,
      streak: d.journal?.streak?.currentStreak || 0,
      entries: d.journal?.streak?.totalEntries || 0,
    ...
    },
    {
      name: 'Brain Dump',
      icon: Target,
      cognitiveLoad: d.brainDump?.cognitiveLoad?.load_percentage || 70,
      pendingItems: d.brainDump?.pendingItems?.length || 0,
    ...
    },
    {
      name: 'Focus Score',
      icon: Zap,
      score: d.focus?.score?.currentScore || 75,
      sessions: d.focus?.score?.totalSessions || 0,
      color: 'text-cyan-600',
      bgColor: 'bg-cyan-500',
    },
    {
      name: 'Emotion State',
      icon: Heart,
      dominant: d.emotion?.stats?.dominantEmotion || 'neutral',
      checkins: d.emotion?.stats?.totalCheckins || 0,
      color: 'text-rose-600',
      bgColor: 'bg-rose-500',
    },
    {
      name: 'Momentum',
      icon: RotateCcw,
      score: d.momentum?.stats?.averageScore || 25,
      dimensions: d.momentum?.stats?.dimensionsInSync || 0,
      color: 'text-teal-600',
      bgColor: 'bg-teal-500',
    },
  ];
